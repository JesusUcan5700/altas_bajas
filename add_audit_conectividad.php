<?php
/**
 * Script para agregar campos de auditoría a la tabla conectividad
 */
...
echo "=== AGREGANDO CAMPOS DE AUDITORÍA A TABLA CONECTIVIDAD ===\n\n";
...
try {
    ...
    $sql1 = "ALTER TABLE conectividad ADD COLUMN fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP";
    $db->createCommand($sql1)->execute();
    echo "✅ Campo fecha_creacion agregado\n";

    // 2. Agregar campo fecha_ultima_edicion  
    echo "📝 Agregando campo fecha_ultima_edicion...\n";
    $sql2 = "ALTER TABLE conectividad ADD COLUMN fecha_ultima_edicion TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP";
    $db->createCommand($sql2)->execute();
    echo "✅ Campo fecha_ultima_edicion agregado\n";

    // 3. Agregar campo ultimo_editor
    echo "📝 Agregando campo ultimo_editor...\n";
    $sql3 = "ALTER TABLE conectividad ADD COLUMN ultimo_editor VARCHAR(100) DEFAULT NULL";
    ...
    $transaction->commit();
    echo "\n🎉 ¡Campos de auditoría agregados exitosamente a la tabla conectividad!\n";

    // Verificar el resultado
    echo "\n=== VERIFICACIÓN FINAL ===\n";
    $columns = $db->createCommand("DESCRIBE conectividad")->queryAll();
