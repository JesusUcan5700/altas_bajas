<?php
// Script para verificar y agregar campos de auditoría a memoria RAM
try {
    $pdo = new PDO('mysql:host=localhost;dbname=altas_bajas', 'root', '');
    echo "Conexión PDO exitosa\n";
    
    // Verificar estructura de la tabla memoria_ram
    $stmt = $pdo->query("SHOW COLUMNS FROM memoria_ram");
    $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
    echo "Campos existentes en memoria_ram: " . implode(', ', $columns) . "\n\n";
    ...
    if (!empty($fieldsToAdd)) {
        ...
        foreach ($fieldsToAdd as $field) {
            if ($field === 'ultimo_editor') {
                $sql = "ALTER TABLE memoria_ram ADD COLUMN $field VARCHAR(100) NULL";
            } else {
                $sql = "ALTER TABLE memoria_ram ADD COLUMN $field TIMESTAMP NULL";
            }
        ...
        $pdo->exec("UPDATE memoria_ram SET fecha_creacion = NOW(), fecha_ultima_edicion = NOW(), ultimo_editor = 'Sistema' WHERE fecha_creacion IS NULL");
    ...
    if ($example) {
        echo "\nEjemplo de registro:\n";
        echo "ID: " . ($example['idMemoriaRam'] ?? 'N/A') . "\n";
